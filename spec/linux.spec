# openRuyi-layout linux spec template for kernel-team-tools/rpm-build.
...
# SHOULD NOT TOUCH
%global kernel_local_version    %{?variant_name:%{variant_name}-}%{kb_klocal}
%global kernel_version          %{?kb_kversion}
%global kernel_full_version     %{kernel_version}-%{kernel_local_version}

%global kernel_make_flags %{?kb_karch:ARCH=%{kb_karch} }%{?kb_cross:CROSS_COMPILE=%{kb_cross} }%{?kb_llvm:LLVM=%{kb_llvm} }%{!?kb_llvm:LD=%{?kb_cross:%{kb_cross}}ld.bfd}
%global modpath /lib/modules/%{kernel_full_version}

%if "%{?kb_suffix}" != ""
    %global arch_suffix %{kb_suffix}
%else
...
%global patchset_release %{?kb_patchset}
...
cp -v "${patchset_dir}/%{?kb_config_name:%{kb_config_name}}%{!?kb_config_name:config.%{_arch}%{arch_suffix}}" .config
rm -rf "${patchset_dir}"
